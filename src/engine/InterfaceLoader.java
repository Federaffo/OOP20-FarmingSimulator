...
public class InterfaceLoader {
	...
	public static ArrayList<Class> getInterfaces(String packageName) throws ClassNotFoundException, IOException {
		...
		for (File directory : dirs) {
			classes.addAll(findInterfaces(directory, packageName));
		}
		return classes;
	}
	...
	private static List<Class> findInterfaces(File directory, String packageName) throws ClassNotFoundException {
		...
		for (File file : files) {
			if (file.isDirectory()) {
				...
				classes.addAll(findInterfaces(file, packageName + "." + file.getName()));
			} else if (file.getName().endsWith(".class")) {
				classes.add(
						Class.forName(packageName + '.' + file.getName().substring(0, file.getName().length() - 6)));
			}
		}
		return classes.stream().filter(x -> x.isInterface()).collect(Collectors.toList());
	}
